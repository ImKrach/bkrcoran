import { FETCH_SURAH, FETCH_SURAH_SUCCESS, FETCH_SURAH_FAILURE, FETCH_EDITION, FETCH_EDITION_SUCCESS, FETCH_EDITION_FAILURE, CHANGE_SURAH, CHANGE_EDITION, PLAY_SURAH, PAUSE_SURAH } from '../constants'
...
export default surahReducer = (state = initialState, action) => {
  ...
  switch (action.type) {
      ...
      case FETCH_SURAH:
        return { ...state, loading: true, name: "", nameTranslated: "", ayahList: [] };
      ...
      case FETCH_SURAH_FAILURE:
        return { ...state, loading: false, error: true, name: "", nameTranslated: "" };
      ...
      case PAUSE_SURAH:
        if (state.loading === false && state.error === false && state.playing === true) {
